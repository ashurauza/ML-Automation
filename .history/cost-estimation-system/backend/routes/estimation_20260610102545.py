...
async def upload_drawing(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    try:
        ...
        response_data = {
            ...
            "filename": filename,
            "extracted_parameters": parameters.model_dump(),
            "ocr_result": {
                "confidence_score": float(ocr_confidence or 0.0),
                "dimensions_found": int(len(ocr_result.get("dimensions", []))),
                "thread_specs": list(ocr_result.get("thread_specs", [])),
                "surface_finish_specs": list(ocr_result.get("surface_finish_specs", [])),
