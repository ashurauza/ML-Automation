        ...
        try:
            ...
            if isinstance(geometric_features, list):
                for feat in geometric_features:
                    if isinstance(feat, str) and "hole" in feat.lower():
            ...
            if isinstance(geometric_features, list):
                for feat in geometric_features:
                    if isinstance(feat, str) and "thread" in feat.lower():
            ...
            if isinstance(geometric_features, list):
                for feat in geometric_features:
                    if isinstance(feat, str) and ("smooth" in feat.lower() or "finish" in feat.lower()):
            ...
            if isinstance(geometric_features, list):
                for feat in geometric_features:
                    if isinstance(feat, str) and "bore" in feat.lower():
    ...
    def _extract_dimensions(self, dimension_list: List[Dict]) -> DimensionData:
        ...
        if dimension_list:
            for idx, dim in enumerate(dimension_list[:6]):  # Limit to 6 dimensions
                ...
                unit = dim.get("unit", "mm") if dim else "mm"
                
                # Assign to appropriate field based on order
                if idx == 0 and value and value > 0:
                    dimensions_dict["length"] = value
                elif idx == 1 and value and value > 0:
                    dimensions_dict["width"] = value
                elif idx == 2 and value and value > 0:
                    dimensions_dict["height"] = value
                elif idx == 3 and value and value > 0:
                    dimensions_dict["diameter"] = value
                elif idx == 4 and value and value > 0:
                    dimensions_dict["radius"] = value
                elif idx == 5 and value and value > 0:
                    dimensions_dict["thickness"] = value
                
                dimensions_dict["unit"] = unit
        
        # Apply smart defaults if no dimensions found
        # Set reasonable default values for missing dimensions
        if not dimensions_dict["length"]:
            dimensions_dict["length"] = 50  # 50mm default
        if not dimensions_dict["width"]:
            dimensions_dict["width"] = 30  # 30mm default
        if not dimensions_dict["height"]:
            dimensions_dict["height"] = 20  # 20mm default
        if not dimensions_dict["diameter"]:
            dimensions_dict["diameter"] = 25  # 25mm default
