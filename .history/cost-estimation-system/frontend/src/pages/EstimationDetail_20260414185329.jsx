...
export default function EstimationDetail() {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Error refreshing estimation');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGenerateEstimate = async () => {
    setLoading(true);
    try {
      const response = await estimationAPI.generateEstimate(estimationId);
      setEstimation(response.data);
      // Also fetch the full details to ensure everything is up to date
      setTimeout(async () => {
  ...
  // Check if estimate needs to be generated
  const needsEstimate = !estimation.total_cost || estimation.total_cost === 0;
      ...
      <Grid container spacing={3}>
          ...
          <Paper sx={{ p: 3 }}>
            ...
            <Card variant="outlined" sx={{ mb: 2 }}>
              <CardContent>
                ...
                <Typography variant="h6">
                  ${(costBreakdown.raw_material_cost || 0).toFixed(2)}
                </Typography>
              </CardContent>
            </Card>
            ...
            <Card variant="outlined" sx={{ mb: 2 }}>
              <CardContent>
                ...
                <Typography variant="h6">
                  ${(costBreakdown.machining_cost || 0).toFixed(2)}
                </Typography>
              </CardContent>
            </Card>
          ...
          <Paper sx={{ p: 3 }}>
            ...
            <Card variant="outlined" sx={{ mb: 2 }}>
              <CardContent>
                ...
                <Typography>
                  {estimation.extracted_parameters?.material_type || 'Not specified'}
                </Typography>
              </CardContent>
            </Card>
            ...
            <Card variant="outlined" sx={{ mb: 2 }}>
              <CardContent>
                ...
                <Typography>
                  {estimation.estimated_cycle_time 
                    ? estimation.estimated_cycle_time.toFixed(2) + ' hours'
                    : 'Not calculated'}
                </Typography>
              </CardContent>
            </Card>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 3 }}>
            ...
            {estimation.manufacturing_operations && estimation.manufacturing_operations.length > 0 ? (
              ...
              <Typography color="textSecondary">No operations defined</Typography>
            )}
          </Paper>
        </Grid>
